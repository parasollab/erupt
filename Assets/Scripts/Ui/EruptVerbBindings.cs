...
namespace Erupt.UiBindings
{
    ...
    /// </remarks>
    public class EruptVerbBindings : MonoBehaviour
    {
        [SerializeField] private TierUiRig rig;
        [SerializeField] private SelectionManager selectionManager;
        [SerializeField] private CollisionObjectsListenerSimple collisionObjectsListener;
        ...
        private void Start()
        {
            ...
            {
                Debug.LogError("EruptVerbBindings: no TierUiRig found — tier 2 verbs will all render disabled.");
                return;
            }

            if (selectionManager == null) selectionManager = FindFirstObjectByType<SelectionManager>();
            if (collisionObjectsListener == null)
                collisionObjectsListener = FindFirstObjectByType<CollisionObjectsListenerSimple>();
        ...
        private void BindObstacleVerbs()
        {
            ...
            menu.Bind("delete", selectable =>
            {
                ...
                undo.Do(new DeleteObstacleCommand(target, PrimitiveTypeOf(target), collisionObjectsListener));
            ...
            menu.Bind("duplicate", selectable =>
            {
                ...
                var create = new CreateObstacleCommand(snapshot, collisionObjectsListener);
                undo.Do(create);
        ...
        // The publisher records an id but not the shape it was built from, so recover the
        // primitive from the mesh — the same sniffing the wrist menu did for duplicate.
